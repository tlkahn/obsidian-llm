...
Desktop-only Obsidian plugin that calls LLMs directly from notes via WASM (`llm-wasm` from `llm-rs`). Two commands: interactive prompt with streaming callout response, and batch `{{llm: ...}}` template processing. TypeScript handles UI; WASM handles LLM API calls.
...
src/
  ...
  settings.ts          PluginSettingTab (API key, model, base URL, system prompt, temperature)
